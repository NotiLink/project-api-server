...
* `keyword` 테이블의 **`keyword`** 컬럼을 **Nullable**로 설계하여 확장성을 확보했습니다.
* 특정 단어뿐만 아니라 **'모든 새 글'** 을 알림 받고 싶어 하는 사용자의 니즈(Wildcard Mode)를 수용하기 위함입니다. `NULL`일 경우 필터링 없이 모든 크롤링 데이터를 해당 유저에게 매핑하도록 구현했습니다.


* `notification` 테이블에서 **사용자 확인용(`is_read`)** 과 **시스템 발송용(`is_sent`)** 상태를 명확히 분리했습니다.
* 이를 통해 **웹 대시보드 알림(UI)** 과 **이메일 발송 프로세스(Background Job)** 가 서로의 상태에 영향을 주지 않고 독립적으로 동작하도록 설계했습니다. (예: 이메일 발송 실패가 웹 알림 확인에 영향을 주지 않음)
